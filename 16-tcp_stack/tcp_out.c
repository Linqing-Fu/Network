...
void tcp_send_packet(struct tcp_sock *tsk, char *packet, int len) 
{
	...
	//markflag
	//add packet into snd_buf
	struct snd_buffer_node *temp = (struct snd_buffer_node *)malloc(sizeof(struct snd_buffer_node));
	temp->packet = (char *)malloc(sizeof(packet));
	memcpy(temp->packet, packet, len);
	temp->len = len;
	list_add_tail(&(temp->list), &(tsk->snd_buffer));
