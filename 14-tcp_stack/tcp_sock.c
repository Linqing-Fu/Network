...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	...
	struct tcp_sock *p = NULL;
	list_for_each_entry(p, &(tcp_established_sock_table[hash_value]), hash_list){
		if(p->local.ip == saddr && p->local.port == sport && p->peer.ip == daddr && p->peer.port == dport){
			return p;
		} else {
			continue;
		}
	}
	printf("Can't find sock in established table!\n");
	return NULL;
}
...
struct tcp_sock *tcp_sock_lookup_listen(u32 saddr, u16 sport)
{
	...
	struct tcp_sock *p = NULL;
	list_for_each_entry(p, &(tcp_listen_sock_table[hash_value]), hash_list){
		if(p->local.port == sport){
			return p;
		} else {
			continue;
		}
	}
	printf("Can't find sock in listen table!\n");
	return NULL;
}
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	int port = tcp_get_port();
	if(port == 0){
		printf("no available port\n");
		return -1;
	}
	int flag = tcp_sock_set_sport(tsk, port);
	if(flag == -1){
		printf("set sport error!\n");
		return -1;
	}
...
void tcp_sock_close(struct tcp_sock *tsk)
{
	fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
	struct tcp_sock *p = NULL;
	struct tcp_sock *q = NULL;
	switch(tsk->state){
		case TCP_LISTEN:
			list_for_each_entry_safe(p, q, &tsk->listen_queue, list){
				list_delete_entry(&p->list);
				if(p->state == TCP_SYN_RECV){
					p->parent = NULL;
					tcp_unhash(p);
					free_tcp_sock(p);
				}
